...
export default function AdminParticipants() {
  ...
  async function load() {
    ...
    prodSnap.docs.forEach(d => { map[d.id] = { id: d.id, ...d.data() } as Product })
    setProducts(map)
  }
  useEffect(() => { load() }, [])
  ...
  function openModal(url: string) { setImgError(false); setModal(url) }
    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          {pendingCount > 0 && <p className="text-orange-500 text-sm font-semibold mt-0.5">{pendingCount} pending review</p>}
        </div>
        <div className="flex gap-1.5 flex-wrap">
          {(['all','pending','approved','rejected'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f}
              {f!=='all' && <span className="ml-1 opacity-60">({participants.filter(p=>p.status===f).length})</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="card-tech overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={7} className="text-center py-16 text-steel-400"><div className="text-3xl mb-2">📋</div>No participants found.</td></tr>
                : filtered.map(p => (
                  <tr key={p.id} className="border-b border-steel-100 hover:bg-steel-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {p.screenshot_url
                        ? <button onClick={() => openModal(p.screenshot_url!)} className="btn-tech btn-ghost px-3 py-1.5 text-xs font-semibold">📸 View</button>
                    ...
                    <td className="px-4 py-3">
                      {p.status==='pending' ? (
                        <div className="flex gap-1.5">
                          <button onClick={() => updateStatus(p.id,'approved')} title="Approve"
                            className="btn-tech px-3 py-1.5 text-xs bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 font-bold">✓</button>
                          <button onClick={() => updateStatus(p.id,'rejected')} title="Reject"
                            className="btn-tech px-3 py-1.5 text-xs bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 font-bold">✗</button>
                        </div>
                      ) : (
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
            <button onClick={() => setModal(null)}
              className="absolute -top-10 right-0 text-white/70 hover:text-white text-sm font-medium flex items-center gap-1.5">✕ Close</button>
            {imgError ? (
              <div className="bg-graphite-700 border border-graphite-500 rounded-xl p-6 text-center">
                ...
                <a href={modal} target="_blank" rel="noreferrer" className="btn-tech btn-primary inline-flex px-5 py-2.5 text-sm">Open URL directly →</a>
              </div>
            ) : (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={modal} alt="Screenshot proof"
